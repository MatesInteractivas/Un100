...
function primeraPregunta(){
    ...
    setNodo(0);
    nuevoTexto(nodo.getTxt(),0);
    if(!reconstruyendo){
	checkAudio(nodo.obj.audio,nodo.getTxt());
	setTimeout(function(){
	    if(hayAudio==false){
		checkMicromundo(nodo.obj,nodo.getTxt().length*d);
	    } else {
		crearBtApagarAudio();
...
function checkFinal(obj){
	...
	if(document.getElementById("ligaCierre")!=null){
	    document.getElementById("ligaCierre").style.display = "block";
	}
	if(ls){
	    localStorage.setItem("respuestas"+tit,"");
	}
...
function actualizarScroll(h){
    ...
    hTot += h;
    //Truco para que scrollTop funcione bien en Adroid: cambiar overflow a 'hidden', set scrollTop, cambiar nuevamente a 'scroll' o 'auto'.
    document.getElementById('chat_globos').style.overflow = 'hidden';
    //document.getElementById('chat_globos').scrollTop = document.getElementById('chat_globos').scrollHeight; //scrollHeight no funciona en Android
...
function hayMatch(){
	...
	setTimeout(function(){
		...
		delay = nodo.nodosResp[numResp].feedback.length*d/2;
	    }
...
function agregarOpciones(arr){
    ...
    for(var i=0;i<arr.length;i++){
	    ...
	    cel2.setAttribute("class","opcMult");
	    divOpciones.appendChild(tabla);
...
function showMicromundo(id,delay){
	...
	div_mm.innerHTML = "<iframe id='iframe_"+mm.id+"' class='micromundo' src=\'micromundos"+strAndroid+"/"+mm.url+"' align='middle' marginheight='0px' marginwidth='0px'></iframe>";
...
function crear_ventana_mm(id){
    ...
    div_mm_frente_int.innerHTML = "<iframe id='iframe_frente_"+mm_frente.id+"' class='micromundo' src=\'micromundos"+strAndroid+"/"+mm_frente.url+"' align='middle' marginheight='0px' marginwidth='0px'></iframe>";
