...
function checkBrowser(){
    ...
    if(strAndroid=="_android" && browser!="Firefox"){
	setScroll();
    }
}
...
function showMicromundo(id,delay){
	...
	//div_mm.innerHTML = "<iframe id='iframe_"+mm.id+"' class='micromundo' src=\'micromundos"+strAndroid+"/"+mm.url+"' align='middle' marginheight='0px' marginwidth='0px' style='overflow:hidden;'></iframe>";
	var reHTTP = /^http/;
	if(reHTTP.test(mm.url)){
	    div_mm.innerHTML = "<iframe id='iframe_"+mm.id+"' class='micromundo' src=\'"+mm.url+"' align='middle' marginheight='0px' marginwidth='0px' style='overflow:hidden;'></iframe>";
	} else {
	    div_mm.innerHTML = "<iframe id='iframe_"+mm.id+"' class='micromundo' src=\'micromundos/"+mm.url+"' align='middle' marginheight='0px' marginwidth='0px' style='overflow:hidden;'></iframe>";
	}
	document.getElementById('micromundo').appendChild(div_mm);
	if(mm.getW()!=-1){
	    var iframe = document.getElementById("iframe_"+mm.id);
	    iframe.style.width = mm.getW()+"px";
	    iframe.style.height = mm.getH()+"px";
	    posicionarMm(mm.id);
	}
	
    } else {
	mm_frente=miMm;
	if(mm_frente.auto && reconstruyendo==false){
...
function revisitMicromundo(id){
    ...
    //Agregrado 6 dic 2013
    if(miMM.reset){
	var idIframe;
	if(miMM.alFrente){
	    idIframe = "iframe_frente_"+id;
	} else {
	    idIframe = "iframe_"+id;
	}
	var hijo = document.getElementById(idIframe);
	var mensajeTipoSet = { type: "set", name: "reset", value: 1 };
	hijo.contentWindow.postMessage(mensajeTipoSet, "*");
	updateMicromundo(idIframe);
    }
}
...
function hideAllMms(){
    ...
    for(var i=0;i<mms.length;i++){
	    mms[i].setAttribute("style","display:none; ");
    }
}
...
//  div_mm_frente_int.innerHTML = "<iframe id='iframe_frente_"+mm_frente.id+"' class='micromundo' src=\'micromundos"+strAndroid+"/"+mm_frente.url+"' align='middle' marginheight='0px' marginwidth='0px' style='overflow:hidden;'></iframe>";
    var reHTTP = /^http/;
    if(reHTTP.test(mm_frente.url)){
	div_mm_frente_int.innerHTML = "<iframe id='iframe_frente_"+mm_frente.id+"' class='micromundo' src=\'"+mm_frente.url+"' align='middle' marginheight='0px' marginwidth='0px' style='overflow:hidden;'></iframe>";
    } else {
	div_mm_frente_int.innerHTML = "<iframe id='iframe_frente_"+mm_frente.id+"' class='micromundo' src=\'micromundos/"+mm_frente.url+"' align='middle' marginheight='0px' marginwidth='0px' style='overflow:hidden;'></iframe>";
    }
    obj_mm_frente[id] = div_mm_frente;
}
